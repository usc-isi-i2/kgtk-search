...
class FindNearestQnodes(Resource):
    valid_property_data_types = {"external-id",
                                 "wikibase-item",
                                 "quantity",
                                 "string",
                                 "url",
    ...
    def get(self):
        ...
        if item == 'qnode':
            ...
            if data_type and data_type.lower() not in self.valid_property_data_types:
                return {'error': f'Invalid data_type, should be one of the following: '
                                 f'{self.valid_property_data_types}'}
            query = es_search.create_property_query(search_term, size=size, query_type=query_type,
                                                    instance_of=instance_of, data_type=data_type)
